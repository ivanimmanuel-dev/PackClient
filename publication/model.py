...
DIST = ROOT / 'site'
DATA = json.loads((ROOT / 'publication/article.json').read_text(encoding='utf-8'))
TITLE, SUBTITLE, AUTHOR = (DATA[k] for k in ('title', 'subtitle', 'author'))
REPORT_TITLE = f'{TITLE}: {SUBTITLE}'
VERSION = re.search(r'^version:\s*(.+)$', (ROOT / 'CITATION.cff').read_text(encoding='utf-8'), re.M).group(1).strip().strip('\"\'')
